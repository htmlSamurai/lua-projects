...
addEventHandler('onClientResourceStart', g_ResRoot,
	function()
		...
		g_dxGUI = {
			ranknum = dxText:create('1', screenWidth/2, screenHeight - 34, false, 'default-bold', 2, 'right'),
			ranksuffix = dxText:create('st', screenWidth/2+12, screenHeight - 34, false, 'default-bold', 2),
			checkpoint = dxText:create('0/0', 0, 0, false, 'default-bold', 2, 'center'),			
			mapdisplay = dxText:create('Map: none', 5, screenHeight-10, false, 'default', 1, 'left'),		
			timepassed = dxText:create('0:00:00', 0, 0, false, 'default-bold', 1, 'center'),
			timeleft = dxText:create('', 0, 0, false, 'default-bold', 1, 'center'),
			speedometr = dxText:create('', 0, 0, false, 'default-bold', 2, 'center')		}
		g_GUI = {
			healthbar = FancyProgress.create(250, 1000, 'img/progress_health_bg.png', screenWidth/2-123/2, screenHeight-26, 123, 30, 'img/progress_health.png', 7, 7, 109, 16),
			--speedbar = FancyProgress.create(0, 1.5, 'img/progress_speed_bg.png', screenWidth/2-123/2, screenHeight-46, 123, 30, 'img/progress_speed.png', 7, 7, 109, 16),
			g_dxGUI['mapdisplay']:color ( 255, 255, 255, 100 ),
		}
		
		g_dxGUI.timepassed:postGUI(true)
		g_dxGUI.timeleft:postGUI(true)
		g_dxGUI.timeleft:boundingBox(screenWidth/2-123/2-216/2, 0, screenWidth/2-123/2, screenHeight*2-20, false)
		g_dxGUI.timepassed:boundingBox(screenWidth/2+123/2, 0, screenWidth/2+123/2+216/2, screenHeight*2-20, false)
		g_dxGUI.checkpoint:boundingBox(screenWidth/2-123/2, 0, screenWidth/2-123/2-216/2, screenHeight*2-34*2, false)
		g_dxGUI.speedometr:boundingBox(screenWidth/2+123/2, 0, screenWidth/2+123/2+216/2, screenHeight*2-34*2, false)

		hideGUIComponents('timeleft', 'healthbar', 'ranknum', 'ranksuffix', 'checkpoint', 'timepassed')
		...
		--speedometr123
		addEventHandler("onClientRender",g_Root, kmh)
		node = xmlLoadFile("settings.xml")
		if node then
				if xmlNodeGetAttribute(node, "setting") == "mph" then
					removeEventHandler("onClientRender",g_Root, kmh)
					addEventHandler("onClientRender",g_Root, mph)
				elseif xmlNodeGetAttribute(node, "setting") == "none" then
					removeEventHandler("onClientRender",g_Root, kmh)
					removeEventHandler("onClientRender",g_Root, mph)
				end
				xmlUnloadFile(node)
		end
	end
...
--создание динамического прямоугольника для HUD, размер которого зависит от количества чекпоинтов.
addEventHandler("onClientRender", root, 
    function()
        local screenW, screenH = guiGetScreenSize()
        
        -- Базовые параметры прямоугольника
        local rectX = screenW/2-123/2-216/2
        local rectY = screenH - 50
        local rectWidth = 225 + 123
        local rectHeight = 50
        
        -- Фиксированные расширения в зависимости от количества чекпоинтов
        if g_Checkpoints then
            local checkpointCount = #g_Checkpoints
            local extraWidth = 0
            
            if checkpointCount >= 100 and checkpointCount < 1000 then
                extraWidth = 50  -- Фиксированное расширение для 100-999 чекпоинтов
            elseif checkpointCount >= 1000 then
                extraWidth = 100  -- Фиксированное расширение для 1000+ чекпоинтов
            end
            
            -- Применяем расширение
            if extraWidth > 0 then
                rectWidth = rectWidth + extraWidth
                rectX = rectX - extraWidth/2  -- Центрируем расширение
            end
        end
        
        dxDrawRectangle(rectX, rectY, rectWidth, rectHeight, tocolor(0, 0, 0, 150), false)
    end
)
...
function launchRace(duration)
	...
	if type(duration) == 'number' then
		showGUIComponents('timeleft', 'timepassed')
		g_dxGUI.timeleft:color(255, 255, 255)
		g_Duration = duration
...
function updateTime()
	...
	g_dxGUI.timeleft:text(msToTimeStr(timeLeft > 0 and timeLeft or 0))
...
function startHurry()
	...
	g_dxGUI.timeleft:color(255, 0, 0)
end
...
function MovePlayerAway.start()
    ...
    MovePlayerAway.posZ = 0 -- fix stream problem during into spectator's mode. Default value - MovePlayerAway.posZ = 34567 + math.random(0,4000)
    MovePlayerAway.rotZ = 0
    MovePlayerAway.health = math.max(1,getElementHealth(element))
    setElementHealth( element, 2000 )
    setElementHealth( g_Me, 90 )
    setElementAlpha( element, 0 ) --set alpha value to 0 for car
    setElementAlpha( g_Me, 0 ) --set alpha value to 0 for ped
...
--set stream memory up to 1gb
function streamMemory1gb_test()
engineStreamingSetMemorySize( 1024 * 1024 * 1024 )
end

addEventHandler("onClientResourceStart", root, streamMemory1gb_test, false)
---
...
function raceTimeout()
	...
	removeEventHandler('onClientRender', g_Root, updateTime, x1)
	if g_CurrentCheckpoint then
		destroyCheckpoint(g_CurrentCheckpoint)
		destroyCheckpoint(g_CurrentCheckpoint + 1)
	g_dxGUI.timeleft:text(msToTimeStr(0))
	
	if g_GUI.hurry then
		Animation.createAndPlay(g_GUI.hurry, Animation.presets.guiFadeOut(500), destroyElement)
		g_GUI.hurry = nil
 	end
	triggerEvent("onClientPlayerOutOfTime", g_Me, showClientImage)
	toggleAllControls(false, true, false)
end
end
...
function createRadarBlips(i)
	...
	checkpoint.blip2 = createBlip(pos[1], pos[2], pos[3], 0, isCurrent and 2 or 1, color[1], color[2], color[3], 50)
...
function destroyCheckpoint(i)
	...
	if checkpoint then
		...
		if isElement(checkpoint.blip) then
		...
		if checkpoint.colshape and isElement(checkpoint.colshape) then
...
addEventHandler( "onClientResourceStart", resourceRoot, function() 
  playSound( "audio/raceon.mp3", false ); 
end ) 
...
function mph()
    ...
    if getElementData(localPlayer, 'race.spectating') then
        ...
        if isElement(cameraTarget) and getElementType(cameraTarget) == "vehicle" then
            target = getVehicleOccupant(cameraTarget, 0)
        else
            target = nil
        end
    end
    ...
    if not isElement(target) then
        return -- Если нет цели, завершить функцию
    end
    
    local car = getPedOccupiedVehicle(target)
    if car then
        local speednumber
        local vehicleType = getVehicleType(car)
        
        if vehicleType == "Plane" or vehicleType == "Boat" or vehicleType == "Helicopter" then
            local vx, vy, vz = getElementVelocity(car)
            speednumber = getDistanceBetweenPoints3D(0, 0, 0, vx, vy, vz) * 180
        else
            local vx, vy, vz = getElementVelocity(car)
            speednumber = getDistanceBetweenPoints3D(0, 0, 0, vx, vy, vz) * 180
        end
        ...
        local speed = tostring(speednumber .. " Kmh")
        local x, y = guiGetScreenSize()
        g_dxGUI.speedometr:text(speed)
        -- dxDrawText( speed, x/2+123/2, 0, x/2+123/2+216/2, y-34, tocolor ( 255, 255, 255, 255 ), 1.5, 'default-bold', "center", "center", false, false, false)
    end
end
...
-- add this should be added to race_client.lua of race resource: 

addEvent('onSpectateRequest', true)
addEventHandler('onSpectateRequest',getRootElement(), function() Spectate.start('auto') end)

-- this is for sending 'spectators' team to spectate mode at race starts
