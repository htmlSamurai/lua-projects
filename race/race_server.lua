...
--manual countdown add
g_StartMode = "auto" -- "auto" или "manual"

addCommandHandler('cd',
    function(player, cmd, mode)
        if not isPlayerInACLGroup(player, g_GameOptions.admingroup) then
            outputChatBox("You don't have permission to change start mode", player, 255, 0, 0)
            return
        end
        
        if mode == "auto" or mode == "manual" then
            g_StartMode = mode
            outputChatBox("Race start mode set to: "..mode, root, 0, 255, 0)
        else
            outputChatBox("Usage: /cd [auto|manual]", player, 255, 255, 0)
        end
    end
)

addCommandHandler('cdstart',
    function(player)
        if not isPlayerInACLGroup(player, g_GameOptions.admingroup) then
            outputChatBox("You don't have permission to start the race", player, 255, 0, 0)
            return
        end
        
        if g_StartMode == "manual" and stateAllowsGridCountdown() then
            gotoState('GridCountdown')
            g_RaceStartCountdown:start()
            outputChatBox("Race started manually by "..getPlayerName(player), root, 0, 255, 0)
        else
            outputChatBox("Can't start race now or not in manual mode", player, 255, 255, 0)
        end
    end
)
...
-- g_RaceStartCountdown:addClientHook(6, 'playSoundFrontEnd', 44)
-- g_RaceStartCountdown:addClientHook(5, 'playSoundFrontEnd', 44)
-- g_RaceStartCountdown:addClientHook(4, 'playSoundFrontEnd', 44)
...
g_RaceStartCountdown:useImages('img/countdown_%d.png', 474, 204)
g_RaceStartCountdown:enableFade(true)
-- g_RaceStartCountdown:addClientHook(6, 'playSound', 'audio/count.mp3')
-- g_RaceStartCountdown:addClientHook(5, 'playSound', 'audio/count.mp3')
-- g_RaceStartCountdown:addClientHook(4, 'playSound', 'audio/count.mp3')
-- g_RaceStartCountdown:addClientHook(3, 'playSound', 'audio/count.mp3')
g_RaceStartCountdown:addClientHook(2, 'playSound', 'audio/count.mp3')
g_RaceStartCountdown:addClientHook(1, 'playSound', 'audio/count.mp3')
g_RaceStartCountdown:addClientHook(0, 'playSound', 'audio/count_go.mp3')
...
function joinHandlerBoth(player)
	...
	if TimerManager.hasTimerFor("spawn") then
		...
		if not player then
            ...
            if howManyPlayersNotReady() == 0 then
				TimerManager.destroyTimersFor("spawn")
                -- Только в автоматическом режиме запускаем отсчет
                if stateAllowsGridCountdown() then
                    if g_StartMode == "auto" then
                        gotoState('GridCountdown')
                        g_RaceStartCountdown:start()
                    else
                        -- В ручном режиме просто ждем команды администратора
                        outputChatBox("All players are ready! Admin can start the race with /cdstart", root, 0, 255, 0)
                    end
                end
    		end
			return
    ...
    playerInfo.startMode = g_StartMode
